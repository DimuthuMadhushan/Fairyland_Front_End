...
const Customerlist = () => {
  ...
  const findCustomer = (username) => {
    if (username === null) {
      ...
      fetch(`http://localhost:8080/customer/detail/${username}`, {
        headers: {
          "Authorization": `Bearer ${Cookies.get('jwt')}`,
        },
      })
        .then((response) => response.json())
        .then((json) => navigate('/custdetail', { state: { json } }))
      ...
      <section className="text-gray-600 body-font mb-0">
        <div className="container px-5 py-0 mx-auto">
          <div className="flex lg:w-2/3 w-full sm:flex-row flex-col mx-auto px-8 sm:space-x-4 sm:space-y-0 space-y-4 sm:px-0 items-end">
            ...
            <button onClick={() => findCustomer(username)} className="text-white bg-teal-500 border-0 py-2 px-8 focus:outline-none hover:bg-gray-400 rounded text-lg">Search</button>
          </div>
        </div>
      </section>
                    ...
                    return (
                      <tr>
                        ...
                        <td class="w-10 text-center border-b-2 border-gray-200">
                          <Button value={customer.username} onClick={(e)=>findCustomer(e.target.value)} className='but'>Find</Button>
                        </td>
                      </tr>
